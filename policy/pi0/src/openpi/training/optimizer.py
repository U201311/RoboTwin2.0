    ...
    def create(
        ...
        schedules = {
            "base": create_cosine_schedule(
                self.warmup_steps_base, self.lr_base, 
                self.decay_steps_base, self.decay_lr_base
            ),
            "moe": create_cosine_schedule(
                self.warmup_steps_moe, self.lr_moe,
                self.decay_steps_moe, self.decay_lr_moe
            ),
            "router": create_cosine_schedule(
                self.warmup_steps_router, self.lr_router,
                self.decay_steps_router, self.decay_lr_router
            ),
        }
        ...
        transforms = {
            ...
            "bias": optax.scale(0.0),  # 禁用optimizer对bias的更新，改为手动更新
        }
        
        # Print configuration for each group with schedule details
        group_configs = [
            ("base", "Base model components", 
             self.lr_base, self.wd_base, self.warmup_steps_base, 
             self.decay_steps_base, self.decay_lr_base),
            ("router", "Router expert layers", 
             self.lr_router, self.wd_router, self.warmup_steps_router,
             self.decay_steps_router, self.decay_lr_router),
        ]
        
        for group_name, description, peak_lr, wd, warmup, decay_steps, final_lr in group_configs:
        ...
        # Print bias group configuration (bias updates are handled manually in train.py)
        print(f"{'bias':>8} group: MoE routing bias (manual updates, lr=0)")
        print()
        ...
        def classify_param_by_path(path):
            ...
            if 'moe' in path and 'gating' in path and 'b_gating' in path:
                # MoE bias parameters get special treatment
                return "bias"
            elif 'moe' in path and 'gating' in path:
        ...
        def state_aware_partition(transforms, label_fn):
            ...
            # Use standard optax.partition (no need for extra_args anymore)
                    ...
                    if isinstance(labels_tree, dict) and isinstance(params_tree, dict):
                        for key in labels_tree:
                            if key in params_tree:
                                current_path = f"{path}/{key}" if path else key
                                collect_group_info(labels_tree[key], params_tree[key], current_path)
                    else:
                        ...
                        # Track bias parameter classification
                
                collect_group_info(param_labels, params_dict)
                
                # Print summary
                total_params = sum(group_counts.values())
                for group in ["base", "moe", "router", "bias"]:
                    if group in group_counts:
                        count = group_counts[group]
                        percentage = (count / total_params) * 100
                        print(f"📦 {group:>8} group: {count:3d} parameters ({percentage:5.1f}%)")
            ...
            def update_fn(updates, state, params=None):
                ...
                result_updates, new_state = base_partition.update(updates_dict, state, params_dict)
        ...
        # Use standard optax.chain (no extra_args needed anymore)
        return optax.chain(
